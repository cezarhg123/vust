/// Simple triangle with different colored vertices and textured rect
...
#[test]
fn texture_and_triangle() {
    let mut glfw = glfw::init(glfw::fail_on_errors!()).unwrap();
    glfw.window_hint(glfw::WindowHint::Resizable(false));
    glfw.window_hint(glfw::WindowHint::ClientApi(glfw::ClientApiHint::NoApi));

    let (window, _) = glfw.create_window(800, 600, "Vust Triangle and Texture Test", glfw::WindowMode::Windowed).unwrap();

    let vust_create_info = VustCreateInfo::default()
        .with_app_name("Vust Triangle and Texture Test")
    ...
    let triangle_buffer = Buffer::builder()
        ...
        .with_data(&[
            -1.0f32, -0.2, 1.0, 0.0, 0.0,
            -0.6, -0.2, 0.0, 1.0, 0.0,
            -0.8, 0.2, 0.0, 0.0, 1.0
        ])
        .build(&mut vust, true);

    let texture_pipeline = GraphicsPipeline::new(
        &vust,
        vust::pipeline::GraphicsPipelineCreateInfo {
    ...
    let rect_buffer = Buffer::builder()
        ...
        .with_memory_location(vk::MemoryPropertyFlags::HOST_VISIBLE | vk::MemoryPropertyFlags::HOST_COHERENT)
        .with_data(&[
            0.4f32, -0.3, 0.0, 0.0, // bottom left
            0.4, 0.3, 0.0, 1.0, // top left
            1.0, 0.3, 1.0, 1.0, // top right
            1.0, -0.3, 1.0, 0.0 // bottom right
        ])
        .build(&mut vust, true);
    ...
    while !window.should_close() {
        ...
        vust.draw(3);
        vust.bind_pipeline(texture_pipeline.handle());
        vust.bind_viewport(vk::Viewport { x: 0.0, y: 0.0, width: 800.0, height: 600.0, min_depth: 0.0, max_depth: 1.0 });
        vust.bind_scissor(vk::Rect2D { offset: vk::Offset2D { x: 0, y: 0 }, extent: vk::Extent2D { width: 800, height: 600 } });
        vust.update_descriptor_set(&texture_pipeline, &[WriteDescriptorInfo::Image { image_view: texture.view(), sampler: texture.sampler() }]);
        vust.bind_descriptor_set(&texture_pipeline);
